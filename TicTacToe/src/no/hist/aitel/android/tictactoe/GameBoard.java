...
public class GameBoard {
	 ...
	 *            Board size
	 * @param lengthToWin
	 *            Number of required squares in row
	 */
	public GameBoard(final int size, final int lengthToWin) {
		if (size < MIN_BOARD_SIZE) {
			throw new IllegalArgumentException(String.format(
					"Minimum board size is %d", MIN_BOARD_SIZE));
		}
		if (lengthToWin < MIN_IN_ROW) {
			throw new IllegalArgumentException(String.format(
					"Minimum length to win is %d", MIN_IN_ROW));
		}
		if (lengthToWin > size) {
			throw new IllegalArgumentException(
					"Length to win can't be larger than board size");
		}
	...
	public GameState put(int x, int y, GamePlayer player) {
		if (x < 0 || x >= board.length) {
			throw new IllegalArgumentException(String.format(
					"x must in range(0,%d)", board.length - 1));
		}
		if (y < 0 || y >= board[x].length) {
			throw new IllegalArgumentException(String.format(
					"y must be in range(0,%d)", board[x].length - 1));
		}
		if (player == previousPlayer) {
			throw new IllegalArgumentException(String.format(
					"Player %s had previous move, can't move again", player));
		}
	...
	public GamePlayer get(int x, int y) {
		if (x < 0 || x >= board.length) {
			throw new IllegalArgumentException(String.format(
					"x must be in range(0,%d)", board.length - 1));
		}
		if (y < 0 || y >= board[x].length) {
			throw new IllegalArgumentException(String.format(
					"y must be in range(0,%d)", board[x].length - 1));
	...
	public GameState getState() {
		for (int top = 0; top <= board.length - lengthToWin; ++top) {
			...
			for (int left = 0; left <= board.length - lengthToWin; ++left) {
				...
				diag1: if (board[top][left] != GamePlayer.EMPTY) {
				...
				diag2: if (board[top][right] != GamePlayer.EMPTY) {
					for (int i = 1; i < lengthToWin; ++i) {
	...
	public String toString() {
		...
		final String separator = Strings.repeat(
				"+" + Strings.repeat("-", fieldWidth), length);
